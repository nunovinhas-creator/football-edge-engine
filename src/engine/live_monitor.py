...
from src.alerts.goal_imminent_detector import GoalImminentDetector
...
def build_premium_snapshot(
    ...
    *,
    ...
    Alerta Live Premium e pelo Goal Imminent Detection, reutilizando a
    odd real já obtida pelo chamador (`bookie_odd`) em vez de deixar
    `build_match_snapshot` cair no fallback `DEFAULT_BOOKIE_ODD`. Não
    decide nem envia nada — apenas monta o snapshot, para ser reutilizado
    por `run_live_pipeline` (Alerta Live Premium + Goal Imminent
    Detection) e por `src.alerts.live_scanner` (que precisa do snapshot
    antes de decidir se sequer tenta o envio).
    """
...
def run_live_pipeline():
    ...
    goal_imminent_detector = GoalImminentDetector()
    ...
    for event in events:
        ...
        if odds_provider is not None and isinstance(prediction, dict):
            try:
                ...
                # Snapshot construído UMA única vez e reutilizado tanto
                # pelo Alerta Live Premium como pelo Goal Imminent
                # Detection — nenhum dos dois recalcula nada, só leem
                # este mesmo MatchSnapshot.
                ...
                # Goal Imminent Detection (12 critérios, independente do
                # motor de apostas — ver src.alerts.goal_imminent_detector).
                # No máximo um alerta deste tipo por jogo.
                goal_imminent_outcome = goal_imminent_detector.evaluate_and_maybe_alert(premium_snapshot)
                if goal_imminent_outcome.sent:
                    print(f"🚨 Alerta Goal Imminent enviado para {home} vs {away}.")
            except Exception as e:
